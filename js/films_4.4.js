...
function createTableFilms(films, genres) {
    ...
    do {
        if (films.length == 0){ //если массив с фильмами пуст
            ...
            break;
        } else { //если массив с фильмами не пуст, по очереди добавляем фильмы в таблицу
            strGenre = ''
            k = i + 1;
            filmStart = films[i][0];
            filmName = films[i][1];
            ...
            table_body.insertAdjacentHTML('beforeend', `<tr class="${trClass}">\
                <td class="movie-list__table-time-b"  id="start_film_${k}">${filmStart}</td>\
                <td class="movie-list__table-film-b"><a title="Кинопоиск" href="${linkFilm}"\
                    target="_blank" id="name_film_${k}">${filmName}</a></td>\
